...
-- CREATE EXTENSION "uuid-ossp" SCHEMA osm;
...
(
    gid uuid NOT NULL UNIQUE DEFAULT osm.uuid_generate_v4(),
    osm_id bigint,
...
insert into osm.forts (osm_id, addr_housenumber, addr_street, addr_city, building, name, geom) 
  (select osm_id, "addr:housenumber", "addr:street", "addr:city", building, name, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
...
select p.osm_id, p."addr:housenumber", p."addr:street", p."addr:city", p.name,
  ...
  from
  (
    select osm_id, "addr:housenumber", "addr:street", "addr:city", name, ST_Collect(geom) as way
...
( select osm_id, "addr:city", area, bay, name, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
...
(
    gid uuid NOT NULL UNIQUE DEFAULT osm.uuid_generate_v4(),
    osm_id bigint,
    addr_flats text,   -- таких данных практически нет в исходниках
    addr_housenumber text,
...
insert into osm.buildings (osm_id, addr_housenumber, addr_street, addr_city, building, construction, disused, historic, landuse, layer, name, operator, ref, religion, service, shop, sport, tourism, geom)

( select osm_id, "addr:housenumber", "addr:street", "addr:city", building,
         construction, disused, historic, landuse, layer, name, operator, ref, religion,
         service, shop, sport, tourism, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
...
(
  with bt as (
    select osm_id, "addr:city" as acity, CAST(nullif(admin_level, '') AS smallint) as admin_level, boundary, landuse, name, place, ref, ST_Collect(geom) as way
    from public.planet_osm_polygon
	...
	and  name is not null and (landuse <> 'allotments') -- добавлено, чтоб не дублировалось,эксперимент
...
UPDATE osm.settlements SET
    addr_city = name;  -- was added 18/10/2023
...
( with sid as (select distinct osm_id from osm.settlements)
  select  osm_id, "addr:city", CAST(nullif(admin_level, '') AS smallint), boundary, landuse, name, place, ref,
         case when ST_IsValid(ST_Multi(ST_MakePolygon(geom))) then ST_Multi(ST_MakePolygon(geom)) else null end,
         case when ST_IsValid(ST_Multi(ST_MakePolygon(geom))) then ST_Multi(ST_MakePolygon(geom)) else null end, true
  from public.planet_osm_line
  where osm_id not in (select osm_id from sid) and "place" in ('allotments','hamlet','locality','neighbourhood','suburb','village') and name not ilike('%жилой комплекс%') and ST_IsClosed(geom) and ST_NPoints (geom) > 3
);
delete  from public.planet_osm_line
where "place" in ('allotments','hamlet','locality','neighbourhood','suburb','village') and name not ilike('%жилой комплекс%') and ST_IsClosed(geom) and ST_NPoints (geom) > 3;
...
-- Строим таблицу РЕГИОНОВ. Их два: СПБ и ЛО
...
(
  with bt as (
    select osm_id, "addr_city" as acity, CAST(nullif(admin_level, null) AS smallint) as admin_level, boundary, name, ref, ST_Collect(geom) as way
    -- from public.planet_osm_polygon
	from public.regions
    where boundary = 'administrative'
    group by osm_id, "addr_city", admin_level, boundary, name, ref
  ),
  p as (
    select *,
         case when ST_IsValid(ST_Multi(ST_CollectionExtract(bt.way,3))) then ST_Multi(ST_CollectionExtract(bt.way,3)) else null end as cway
...
DO $$
    BEGIN
	RAISE INFO 'Построили таблицу РЕГИОНОВ.';
    END;
$$;
...
ALTER TABLE osm.districts OWNER to postgres;

DROP INDEX IF EXISTS osm.addr_city_regions,osm.addr_district_regions,osm.addr_reg_regions,osm.name_regions,osm.geonim_name_regions,osm.geonim_type_regions;

DROP INDEX IF EXISTS osm.geom_districts, osm.geom_uniq_districts;
CREATE INDEX geom_districts ON osm.districts USING GIST ( geom );
CREATE INDEX geom_uniq_districts ON osm.districts USING GIST ( geom_uniq );

CREATE INDEX addr_city_regions ON osm.districts USING HASH (addr_city);
CREATE INDEX addr_district_regions ON osm.districts USING HASH (addr_district);
CREATE INDEX addr_reg_regions ON osm.districts USING HASH (addr_reg);
CREATE INDEX name_regions ON osm.districts USING HASH (name);
CREATE INDEX geonim_name_regions ON osm.districts USING HASH (geonim_name);
CREATE INDEX geonim_type_regions ON osm.districts USING HASH (geonim_type);

insert into osm.districts (osm_id, addr_city, admin_level, boundary, name, ref, geom, geom_uniq)
(
  with bt as (
    select osm_id, "addr:city" as acity, CAST(nullif(admin_level, '') AS smallint) as admin_level, boundary, name, ref, ST_Collect(geom) as way
    from public.planet_osm_polygon
...
( select osm_id, "addr:city", landuse, layer, name, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_polygon
...
( select osm_id, "addr:city", landuse, layer, name, ref, waterway, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_line
...
( select osm_id, "addr:street", "addr:city", highway, name, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_polygon
...
(
  with bt as (
    select osm_id, "addr:street" as astreet, "addr:city" as acity, boundary, landuse, name, place, ref, ST_Collect(geom) as way
  from public.planet_osm_polygon
...
(
  ...
  from
  (
    select osm_id, "addr:street", "addr:city", boundary, landuse, name, place, ref, ST_Collect(geom) as way
    from public.planet_osm_polygon
...
(
  ...
  (
    select  osm_id, "addr:street", "addr:city", boundary, landuse, name, place, ref,
          case when ST_IsValid(ST_Collect(ST_MakePolygon(geom))) then ST_Collect(ST_MakePolygon(geom)) else null end as way
    from public.planet_osm_line
    where "boundary" is not null and (name ilike '%заказник%' or name ilike '%исторический%' or name ilike '%парк%' ) and ST_IsClosed(geom)  and ST_NPoints (geom) > 3
...
(
  select osm_id, "addr:street", "addr:city", name, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_polygon
  where "place" ='square'
);
...
(
  ...
  select osm_id, "addr:street", "addr:city", name, ref,
   case when ST_IsValid(ST_Collect(ST_MakePolygon(geom))) then ST_Collect(ST_MakePolygon(geom)) else null end as way
  from public.planet_osm_line
  where "place" ='square' and ST_IsClosed(geom)  and ST_NPoints (geom) > 3
  group by osm_id, "addr:street", "addr:city", admin_level, boundary, landuse, name, place, ref
  ) as p
);
...
( select osm_id, "addr:street", "addr:city", layer, name, operator, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
...
( select osm_id, "addr:housenumber", "addr:street", "addr:city", name, operator, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
...
( select osm_id, "addr:housenumber", "addr:street", "addr:city", name, operator, ref, service, shop, tourism, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
...
( select osm_id, "addr:housenumber", "addr:street", "addr:city", name, operator, ref, religion, service, shop, sport, tourism, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
...
( select osm_id,  "addr:housenumber", "addr:street", "addr:city", construction, disused, landuse, name, operator, ref, religion, tourism, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_polygon
...
( select  osm_id, "addr:housenumber", "addr:street", "addr:city", name, operator, ref, religion, tourism, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_point
  where amenity in ('college','kindergarten','library','archive','public_bookcase','school','music_school','driving_school','language_school','university','research_institute')
);
...
( select osm_id, "addr:city", name, place, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
...
( with sid as (select distinct osm_id from osm.island)
  select osm_id, "addr:city", name, place, ref, case when ST_IsValid(ST_Multi(ST_MakePolygon(geom))) then ST_Multi(ST_MakePolygon(geom)) else null end
  from public.planet_osm_line
  where osm_id not in (select osm_id from sid) and "place" in ('island','islet') and name is not null and ST_IsClosed(geom)  and ST_NPoints (geom) > 3
);
...
( select osm_id, "addr:housenumber", "addr:street", "addr:city", boundary, construction, disused, highway, historic, landuse, layer, name, "natural", operator, place, ref, religion, service, shop, sport, tourism, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_polygon
 where planet_osm_polygon.landuse is not null
);


-- delete from public.planet_osm_polygon where planet_osm_polygon.landuse is not null;
...
( with sid as (select distinct osm_id from osm.landuse)
  select osm_id, "addr:housenumber", "addr:street", "addr:city", boundary, construction, disused, highway, historic, landuse, layer, name, operator, place, ref, religion, service, shop, sport, tourism,
     case when ST_IsValid(ST_Multi(ST_MakePolygon(geom))) then ST_Multi(ST_MakePolygon(geom)) else null end
  from public.planet_osm_line
  where osm_id not in (select osm_id from sid) and ST_IsClosed(geom) and ST_NPoints (geom) > 3 and "natural" is null and "man_made" is null
);


--  delete from public.planet_osm_line where ST_IsClosed(geom)  and ST_NPoints (geom) > 3 and "natural" is null and "man_made" is null;
...
( select osm_id, "addr:housenumber", "addr:street", "addr:city", boundary, construction, disused, highway, historic, landuse, layer, name, operator, place, ref, religion, service, shop, sport, tourism, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_line
...
-- Делим таблицу с линиями на дороги и недороги
CREATE TABLE IF NOT EXISTS public.planet_osm_roads AS (select * from planet_osm_line where planet_osm_line.highway is not null);

delete from planet_osm_line where planet_osm_line.highway is not null;
...
------------------------------------------------------
------------------------------------------------------
-- Тут начинается блок, заполняющий таблицу route. 
-- Пропускаем его, поскольку в исходных данных нет таког тега,  в том числе в поле other_tags
-- Плюс. Нет пока четкого пониманимания как именно используется в дальнейшем
...
(select osm_id, 'subway', "addr:city", construction, disused, layer, name, operator, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
    from public.planet_osm_line
    where railway = 'subway' or construction = 'subway' or disused = 'subway');


delete from public.planet_osm_line where railway = 'subway' or construction = 'subway' or disused = 'subway';
...
delete from public.planet_osm_line where railway is not null and tunnel = 'yes';
...
(with sid as(select distinct osm_id from osm.route)
    select osm_id, 'subway', "addr:city", construction, disused, layer, name, operator, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
    from public.planet_osm_line
    where osm_id not in(select osm_id from sid)and railway = 'subway');
delete from public.planet_osm_line where railway = 'subway';
...
(select osm_id, 'tram', "addr:street", "addr:city", construction, disused, highway, historic, name, operator, ref, service, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
    from public.planet_osm_line
    where railway = 'tram' or construction = 'tram' or disused = 'tram');
delete from public.planet_osm_line where railway = 'tram' or construction = 'tram' or disused = 'tram';

insert into osm.route(osm_id, route_type, addr_street, addr_city, construction, disused, highway, historic, name, operator, ref, service, geom)
(with sid as(select distinct osm_id from osm.route)
    select osm_id, 'tram', "addr:street", "addr:city", construction, disused, highway, historic, name, operator, ref, service, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
    from public.planet_osm_line
    where osm_id not in(select osm_id from sid)and railway in('tram', 'tram_cancelled', 'tram_temporary_cancelled'));
delete from public.planet_osm_line where railway in('tram', 'tram_cancelled', 'tram_temporary_cancelled');
...
( select osm_id, "addr:city", construction, disused, highway, layer, name, operator, ref, service, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
...
( select osm_id, "addr:housenumber", "addr:street", "addr:city", layer, railway, name, operator, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_point
...
( select osm_id, "addr:city", construction, highway, bridge, layer, lanes, name, oneway, operator, ref, route, service, surface, maxspeed, tunnel, width, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_roads
...
( select osm_id, "addr:housenumber", "addr:street", "addr:city", distance, layer, name, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_point
...
( select osm_id, "addr:housenumber", "addr:street", "addr:city", landuse, layer, name, ref, tourism, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_line
...
( select osm_id, "addr:city", construction, disused, layer, name, operator, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_line
...
( select osm_id, "addr:housenumber", "addr:street", "addr:city", building, construction, disused, name, operator, ref, case when ST_IsValid(ST_Multi(geom)) then ST_Multi(geom) else null end
  from public.planet_osm_point
  where "power" is not null
);
...
DO $$
    ...
    END;
$$;
------------- Реальные данные единичны. Конец. 
-------------
*/
...
--                  select gid::int, admin_level, name, 'handadd',
insert into osm.regions (admin_level, name, hand, geom, geom_uniq)
                  select admin_level, name, 'handadd',
...
insert into osm.settlements (admin_level, name, hand, geom, geom_uniq)
                      select admin_level, name, 'handadd',					  
...
insert into osm.roads (name, hand, geom)
                select name, roadtype, case when ST_IsValid(geom) then geom else null end
                from hand.roads;
...
insert into osm.water (name, hand, geom)
                      select name, 'handadd', case when ST_IsValid(geom) then geom else null end
                      from hand.water;
...
insert into osm.square (name, hand, geom)
                 select name, 'handadd', case when ST_IsValid(geom) then geom else null end
                 from hand.square;
...
insert into osm.island (name, hand, geom)
                 select name, 'handadd', case when ST_IsValid(geom) then geom else null end
                 from hand.island;
...
insert into osm.forts (addr_housenumber, addr_street, name, hand, geom)
                select addr_housenumber, addr_street, name, 'handadd', case when ST_IsValid(geom) then geom else null end
                from hand.forts;
...
insert into osm.buildings (addr_housenumber, addr_street, building, name, hand, geom)
                    select addr_housenumber, addr_street, buildingtype, name, 'handadd', case when ST_IsValid(geom) then geom else null end
                    from hand.buildings;
DO $$
    BEGIN
...
insert into osm.railway_point (addr_housenumber, addr_street, name, hand, geom)
                        select addr_housenumber, addr_street, name, 'handadd', case when ST_IsValid(geom) then geom else null end
...
insert into osm.milestone_point (distance, name, hand, geom)
                          select distance, name, 'handadd', case when ST_IsValid(st_multi(geom)) then st_multi(geom) else null end
...
begin
 for rec in select * from hand.torename --where fieldname is null limit 10
  LOOP
  if rec.fieldname is null then
  	RAISE INFO 'null объекты в таблицe %,  c id= %',rec.tablename, rec.osm_id;
  else
    if rec.newname is null then
 		execute 'Update '||coalesce(rec.tablename,'')||' set '||coalesce(rec.fieldname,'')||'= null WHERE osm_id = '||coalesce(rec.osm_id,0);
	else
  		execute 'Update '||coalesce(rec.tablename,'')||' set '||coalesce(rec.fieldname,'')||'='''||rec.newname||''' WHERE osm_id = '||coalesce(rec.osm_id,0);
	end if;
  end if;	
...
/*
-------------------------------------------------------------------------------------------------------
--- Вряд ли этот кусок имеет смысл, поскольку в исходных данных нет геонимов
DO $$
    BEGIN
	update osm.settlements set
               name_orig  = name,
